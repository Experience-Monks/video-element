module.exports = function(url, options, callback) {
	...
	if (format) {
		...
		if (options.parameters.autoplay) player.setAttribute('autoplay','');
		if (options.parameters.controls) player.setAttribute('controls','');
		if (options.parameters.loop) player.setAttribute('loop','');
		...
		if (options.id) player.setAttribute('id',options.id);
